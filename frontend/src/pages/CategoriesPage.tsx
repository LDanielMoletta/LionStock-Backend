...
const CategoriesPage = () => {
  ...
  const [form, setForm] = useState<CreateCategoryPayload & { _id?: string }>({ name: '', description: '' });
  ...
  const load = async () => {
    try {
      ...
      setError((err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Erro ao carregar categorias.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (c: Category) => { setForm({ _id: c._id, name: c.name || '', description: c.description || '' }); setModal('edit'); setFormError(''); };
  const openDelete = (c: Category) => { setForm(c); setModal('delete'); };
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal === 'create') {
        const payload: CreateCategoryPayload = { name: form.name, description: form.description };
      ...
      {(modal === 'create' || modal === 'edit') ? (
        <Modal title={modal === 'create' ? 'Nova Categoria' : 'Editar Categoria'} onClose={() => setModal(null)}>
          <div className="space-y-3">
            <div><label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">Nome da Categoria</label><input placeholder="Ex: Eletrônicos" value={form.name} onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((p) => ({ ...p, name: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-lion-blue" /></div>
            <div><label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">Descrição</label><textarea placeholder="Descrição da categoria" value={form.description} onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setForm((p) => ({ ...p, description: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-lion-blue" rows={2} /></div>
