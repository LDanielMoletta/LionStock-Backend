...
const emptyForm: ProductFormState = { _id: '', sku: '', name: '', description: '', category: '', supplier: '', quantity: 0, unitPrice: 0 };
...
const ProductsPage = () => {
  ...
  const load = async () => {
    try {
      ...
      setError((err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Erro ao carregar produtos.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openDelete = (p: Product) => { editingProductId = p._id; setForm({ ...emptyForm, _id: p._id, name: p.name }); setModal('delete'); };
  ...
  const set = (field: keyof ProductFormState) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => 
...
<td className="px-4 py-3 text-slate-600">{typeof p.category === 'object' ? p.category?.name || '-' : '-'}</td>
<td className="px-4 py-3 text-slate-600">{typeof p.supplier === 'object' ? p.supplier?.name || '-' : '-'}</td>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <select value={form.category} onChange={set('category')} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-lion-blue focus:ring-2 focus:ring-lion-blue/10">
                ...
                {categories.map((c) => <option key={typeof c === 'object' ? c._id : ''} value={typeof c === 'object' ? c._id : ''}>{typeof c === 'object' ? c.name : ''}</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={form.supplier} onChange={set('supplier')} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-lion-blue focus:ring-2 focus:ring-lion-blue/10">
                ...
                {suppliers.map((s) => <option key={typeof s === 'object' ? s._id : ''} value={typeof s === 'object' ? s._id : ''}>{typeof s === 'object' ? s.name : ''}</option>)}
              </select>
            </div>
            <div>
