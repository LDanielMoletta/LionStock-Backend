...
import { Users as Users2, Pencil, Trash2, Plus } from 'lucide-react';
import PageHeader from '../components/common/PageHeader';
import StateView from '../components/common/StateView';
import Modal from '../components/ui/Modal';
import { extractListData } from '../services/api';
import { userService, User, CreateUserPayload, UpdateUserPayload, UserRole } from '../services/userService';
...
const UsersPage = () => {
  ...
  const [form, setForm] = useState<UserFormState>({ name: '', email: '', password: '', role: 'viewer', active: true });
  ...
  const load = async () => {
    try {
      ...
      setError((err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Erro ao carregar usuários.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (u: User) => { setForm({ _id: u._id, name: u.name || '', email: u.email || '', password: '', role: u.role || 'viewer', active: typeof u.active === 'boolean' ? u.active : true }); setModal('edit'); setFormError(''); };
  const openDelete = (u: User) => { setForm({ _id: u._id, name: u.name, email: u.email, password: '', role: u.role, active: u.active }); setModal('delete'); };
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal === 'create') {
        const payload: CreateUserPayload = { name: form.name, email: form.email, password: form.password || '', role: form.role, active: form.active };
      ...
      {(modal === 'create' || modal === 'edit') ? (
        <Modal title={modal === 'create' ? 'Novo Usuário' : 'Editar Usuário'} onClose={() => setModal(null)}>
          <div className="space-y-3">
            <div><label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">Nome do Usuário</label><input placeholder="Nome completo" value={form.name} onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((p) => ({ ...p, name: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-lion-blue" /></div>
            <div><label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">E-mail</label><input placeholder="email@exemplo.com" type="email" value={form.email} onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((p) => ({ ...p, email: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-lion-blue" /></div>
            <div><label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">Senha</label>
            {modal === 'create' ? (
              <input placeholder="Mínimo 6 caracteres" type="password" value={form.password} onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((p) => ({ ...p, password: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-lion-blue" />
            ) : (
              <input placeholder="Deixe em branco para manter" type="password" value={form.password} onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((p) => ({ ...p, password: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-lion-blue" />
            )}
            </div>
            <div><label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">Perfil</label>
            <select value={form.role} onChange={(e: ChangeEvent<HTMLSelectElement>) => setForm((p) => ({ ...p, role: e.target.value as UserRole }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-lion-blue">
            ...
            {modal === 'edit' ? (
              <label className="flex items-center gap-2 text-sm text-slate-700">
                <input type="checkbox" checked={form.active} onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((p) => ({ ...p, active: e.target.checked }))} className="rounded border-slate-300" />
                Usuário ativo
              </label>
            ) : null}
